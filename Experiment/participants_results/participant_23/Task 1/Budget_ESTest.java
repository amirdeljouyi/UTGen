...
//    Budget budget = new Budget(fixedClock);
  ...
  public void testReplaceWithItself() throws Throwable {
    ...
    Clock fixedClock = Clock.fixed(Instant.parse("2024-02-22T10:15:30.00Z"), ZoneId.of("UTC"));
    Budget budget = new Budget();
    Budget budget2 = new Budget(fixedClock);

    // When we replace it with itself
    budget.replaceWith(budget2);

    // Then the calories per hour should be 0.0
    assertEquals(budget2.getCaloriesPerHour(), budget.getCaloriesPerHour(), 0.01);
    assertEquals(budget2.getStartOfDay().toTime(), budget.getStartOfDay().toTime(), 0.01);
    assertEquals(budget2.getEndOfDay().toTime(), budget.getEndOfDay().toTime(), 0.01);
    assertEquals(budget2.getInterval(), budget.getInterval(), 0.01);
  }
