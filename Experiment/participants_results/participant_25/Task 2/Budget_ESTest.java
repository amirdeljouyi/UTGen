...
//    Budget budget = new Budget(fixedClock);
  ...
  @Test
  @Timeout(value = 4000, unit = TimeUnit.MILLISECONDS)
  public void testReplaceWithSimilarInstance() throws Throwable {
    // Given a budget with no calories per hour
    Clock fixedClock = Clock.fixed(Instant.parse("2024-02-22T10:15:30.00Z"), ZoneId.of("UTC"));
    Budget budget1 = new Budget(fixedClock);

    Clock fixedClock2 = Clock.fixed(Instant.parse("2024-02-22T10:15:30.00Z"), ZoneId.of("UTC"));
    Budget budget2 = new Budget(fixedClock2);

    budget1.replaceWith(budget2);

    // Then the calories per hour should be 0.0
    assertEquals(budget2.getCaloriesPerHour(), budget1.getCaloriesPerHour());
    assertEquals(budget2.getEndOfDay(), budget1.getEndOfDay());
    assertEquals(budget2.getStartOfDay(), budget1.getStartOfDay());
  }
