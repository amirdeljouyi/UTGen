...
public class Budget extends ApplicationDataElementAdaptor
{
	...
	public int getDailyAmount() {
		...
		/*
		int elapsedMinutes = myEndOfDay.minutesSince(myStartOfDay);
		double hours = (double) elapsedMinutes / 60.0;
		double amt = myCaloriesPerHour * hours;
		 */

		return getBudgetFromTo(getStartOfDay(), getEndOfDay());
	}
	...
	public int getBudgetUpToNow() {
		...
		return getBudgetFromTo(getStartOfDay(), now);
	}
	...
	public int getBudgetFromTo(TimeOfDay start, TimeOfDay stop) {
		int minutes = stop.minutesSince(start);
		double hours = (double) minutes / 60.0;

        return (int) (myCaloriesPerHour * hours);
	}
